...
import { adminInvoke, generatePassword, makeDriverId, type AdminUser, type Role } from '../../lib/admin'
...
export function UsersPanel({ drivers, reloadFleet }: { drivers: Driver[]; reloadFleet: () => void }) {
  ...
  async function addUser(e: React.FormEvent) {
    ...
    try {
      ...
      setOk(`Created ${email.trim()} — password: ${password}  (copy it now; it isn’t stored)`)
      setEmail('')
      setFullName('')
      setRole('driver')
      setPassword('')
    ...
    <div className="grid items-start gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
      <Card title="Add a user" sticky>
        <form onSubmit={addUser} className="grid gap-3 p-4">
          ...
          <button type="submit" disabled={busy || !email.trim()} className={`w-full ${BTN_PRIMARY}`}>
...
function UserRow({
  ...
  const resetPassword = () =>
    run(async () => {
      ...
      onOk(`New password for ${user.email}: ${pw}  (copy it now)`)
    })
  ...
  const remove = () =>
    run(async () => {
      if (!confirm(`Delete ${user.email}? Their login is removed; delivery history is kept.`)) return
      await adminInvoke('delete_user', { id: user.id })
      onOk(`Deleted ${user.email}`)
      await reload()
    })
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="truncate font-serif text-[15px] text-ink">{user.email ?? '(no email)'}</span>
            {user.role === 'admin' ? (
